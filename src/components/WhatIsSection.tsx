...
export default function WhatIsSection() {
  ...
  return (
    <section id="what-is" className="bg-ug-page py-24 md:py-32 relative overflow-hidden">
      <div ref={ref} className="relative z-10 max-w-6xl mx-auto px-6">
        ...
        <div className="max-w-3xl mb-16">
          ...
          <motion.h2 variants={FU(0.08)} initial="hidden" animate={inView ? "visible" : "hidden"}
            className="font-serif text-5xl md:text-6xl font-bold text-ug-ink leading-tight mb-4">
            {t.title}
          </motion.h2>
          <motion.p variants={FU(0.14)} initial="hidden" animate={inView ? "visible" : "hidden"}
            className="text-ug-green font-serif text-lg italic mb-6">
            {t.subtitle}
          </motion.p>
          <motion.p variants={FU(0.2)} initial="hidden" animate={inView ? "visible" : "hidden"}
            className="text-ug-muted text-lg leading-relaxed">
            {t.intro}
          </motion.p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-14">
          {steps.map((s, i) => (
            <motion.div key={i} variants={FU(i * 0.1)} initial="hidden" animate={inView ? "visible" : "hidden"}
              ...
              <div className="font-serif text-4xl font-bold text-ug-green/20 mb-4 leading-none">{s.n}</div>
              <h3 className="font-serif text-base font-bold text-ug-ink mb-2 group-hover:text-ug-green transition-colors">{s.title}</h3>
              <p className="text-ug-muted text-sm leading-relaxed">{s.desc}</p>
            </motion.div>
          ))}
        </div>
        ...
        <motion.div variants={FU(0.3)} initial="hidden" animate={inView ? "visible" : "hidden"}
          className="p-8 rounded-3xl bg-ug-green/[0.06] border border-ug-green/20">
          <h3 className="text-ug-green font-bold text-sm mb-3">{t.whyTitle}</h3>
          <p className="text-ug-muted leading-relaxed">{t.whyDesc}</p>
        </motion.div>
      </div>
    </section>
  );
