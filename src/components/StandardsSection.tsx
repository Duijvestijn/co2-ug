        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {t.standards.map((s, i) => (
            <motion.div
              ...
              className="group p-6 rounded-2xl bg-white shadow-sm border border-black/[0.06] hover:shadow-lg transition-all"
            >
              {/* Code badge */}
              <div className="flex items-start justify-between mb-4">
                <span className="px-3 py-1 rounded-lg bg-ug-green/10 border border-ug-green/20 text-ug-green font-mono text-xs font-bold tracking-wide">
                  {s.code}
                </span>
                <span className="text-[10px] font-medium text-ug-muted text-right leading-snug max-w-[110px]">
                  {s.stat}
                </span>
              </div>

              {/* Name */}
              <h3 className="text-ug-ink font-semibold text-sm mb-2 group-hover:text-ug-green transition-colors leading-snug">
                {s.name}
              </h3>

              {/* Description */}
              <p className="text-ug-muted text-sm leading-relaxed">
                {s.desc}
              </p>

              {/* Bottom accent line */}
              <div className="mt-5 h-px bg-gradient-to-r from-ug-green/40 to-transparent" />
            </motion.div>
          ))}
        </div>
      </div>
